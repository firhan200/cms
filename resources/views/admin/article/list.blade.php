            	...
            	<table class="table table-hover">
            		<thead>
            			<tr>
            				<th>No</th>
            				<th>
                                <a href="{{ url('admin/'.$objectName.'/?keyword='.$keyword.'&is_deleted='.$is_deleted.'&sort_by=title&order_type='.$order_type) }}">Title</a>
                            </th>
            				<th>
                                <a href="{{ url('admin/'.$objectName.'/?keyword='.$keyword.'&is_deleted='.$is_deleted.'&sort_by=summary&order_type='.$order_type) }}">Summary</a>
                            </th>
            				<th>
    	        					...
    	        					@if($is_deleted==0)
    		        					<div class="dropdown">
    									  ...
    									  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
    									    ...
    									    <a class="dropdown-item confirm-modal" data-content="delete {{ $obj->title }} ?" data-url="{{ url('/admin/'.$objectName.'/delete/'.$obj->id.'/1') }}" href="#!" data-toggle="modal" data-target="#confirmModal"><i class="fa fa-remove"></i> Delete</a>
    									  </div>
    								@else
                                        <a href="#!" class="btn btn-sm btn-success confirm-modal" data-content="restore {{ $obj->title }} ?" data-url="{{ url('/admin/'.$objectName.'/delete/'.$obj->id.'/0') }}" data-toggle="modal" data-target="#confirmModal">Restore</a>
    									<a href="#!" class="btn btn-sm btn-danger confirm-modal" data-content="delete permanent {{ $obj->title }} ?" data-url="{{ url('/admin/'.$objectName.'/deletePermanent/'.$obj->id) }}" data-toggle="modal" data-target="#confirmModal">Delete</a>
    							  	@endif
    								</div>
    	        				</td>
    	        			</tr>
